...
public class StubOperator extends OperatorProcessor
{
    ...
    private void checkStringOperands(List<COSBase> arguments, int length) throws ContentStreamException
    {
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            if (arg instanceof COSName && ((COSName) arg).getName().length() > PreflightConstants.MAX_NAME_SIZE)
            {
                throw createLimitError(PreflightConstants.ERROR_SYNTAX_NAME_TOO_LONG, "A Name operand is too long");
            }

            if (arg instanceof COSString && ((COSString) arg).getString().getBytes().length > PreflightConstants.MAX_STRING_LENGTH)
            {
    ...
    private void checkArrayOperands(List<COSBase> arguments, int length) throws ContentStreamException
    {
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            if (((COSArray) arg).size() > PreflightConstants.MAX_ARRAY_ELEMENTS)
            {
                throw createLimitError(PreflightConstants.ERROR_SYNTAX_ARRAY_TOO_LONG, "Array has " + ((COSArray) arg).size()
                        + " elements");
            }
        }
    }
    ...
    private void checkNumberOperands(List<COSBase> arguments, int length) throws ContentStreamException
    {
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            if (arg instanceof COSFloat
                    && (((COSFloat) arg).doubleValue() > PreflightConstants.MAX_POSITIVE_FLOAT || ((COSFloat) arg).doubleValue() < PreflightConstants.MAX_NEGATIVE_FLOAT))
            {
    ...
    private void checkTagAndPropertyOperands(List<COSBase> arguments) throws ContentStreamException
    {
        ...
        if (arg2 instanceof COSName && ((COSName) arg2).getName().length() > PreflightConstants.MAX_NAME_SIZE)
        {
            throw createLimitError(PreflightConstants.ERROR_SYNTAX_NAME_TOO_LONG, "A Name operand is too long");
        }

        if (arg2 instanceof COSString && ((COSString) arg2).getString().getBytes().length > PreflightConstants.MAX_STRING_LENGTH)
        {
            throw createLimitError(PreflightConstants.ERROR_SYNTAX_LITERAL_TOO_LONG, "A String operand is too long");
        }

        if (arg2 instanceof COSDictionary && ((COSDictionary) arg2).size() > PreflightConstants.MAX_DICT_ENTRIES)
        {
            throw createLimitError(PreflightConstants.ERROR_SYNTAX_TOO_MANY_ENTRIES, "Dictionary has " + ((COSDictionary) arg2).size()
                    + " entries");
        }
    }
    ...
    private ContentStreamException createInvalidArgumentsError()
    {
        ...
        ex.setErrorCode(PreflightConstants.ERROR_SYNTAX_CONTENT_STREAM_INVALID_ARGUMENT);
        return ex;
    }
