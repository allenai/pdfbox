...
package org.allenai.pdfbox.preflight.parser;
...
import org.allenai.pdfbox.cos.COSNull;
import org.allenai.pdfbox.cos.COSStream;
import org.allenai.pdfbox.pdfparser.PDFParser;
...
import org.allenai.pdfbox.cos.COSFloat;
...
import org.allenai.pdfbox.io.RandomAccessBufferedFileInputStream;
import org.allenai.pdfbox.pdfparser.PDFObjectStreamParser;
import org.allenai.pdfbox.pdfparser.XrefTrailerResolver.XRefType;
import org.allenai.pdfbox.pdmodel.PDDocument;
import org.allenai.pdfbox.preflight.Format;
...
import org.allenai.pdfbox.preflight.exception.SyntaxValidationException;
...
public class PreflightParser extends PDFParser
{
    ...
    protected void checkStreamKeyWord() throws IOException
    {
        ...
        {
            addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_STREAM_DELIMITER,
    ...
    protected COSArray parseCOSArray() throws IOException
    {
        ...
        if (result != null && result.size() > PreflightConstants.MAX_ARRAY_ELEMENTS)
        {
            addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_ARRAY_TOO_LONG, "Array too long : " + result.size()));
        }
        return result;
    }
    ...
    protected COSName parseCOSName() throws IOException
    {
        ...
        if (result != null && result.getName().getBytes().length > PreflightConstants.MAX_NAME_SIZE)
        {
            addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_NAME_TOO_LONG, "Name too long: " + result.getName()));
        }
        return result;
    }
    ...
    protected COSString parseCOSString() throws IOException
    {
        ...
        if (nextChar == '<')
        {
            do
            {
                ...
                if (nextChar != '>')
                {
                    ...
                    {
                        addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_HEXA_STRING_INVALID,
        ...
        if (count % 2 != 0)
        {
            addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_HEXA_STRING_EVEN_NUMBER,
        ...
        if (result.getString().length() > PreflightConstants.MAX_STRING_LENGTH)
        {
            addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_HEXA_STRING_TOO_LONG, "Hexa string is too long at offset "+source.getPosition()));
        }
        return result;
    }
    ...
    protected COSBase parseDirObject() throws IOException
    {
        ...
        if (result instanceof COSNumber)
        {
            ...
            if (number instanceof COSFloat)
            {
                ...
                if (real > PreflightConstants.MAX_POSITIVE_FLOAT || real < PreflightConstants.MAX_NEGATIVE_FLOAT)
                {
                    addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_NUMERIC_RANGE,
        ...
        if (result instanceof COSDictionary)
        {
            ...
            if (dic.size() > PreflightConstants.MAX_DICT_ENTRIES)
            {
                addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_TOO_MANY_ENTRIES, "Too Many Entries In Dictionary at offset "+source.getPosition()));
            }
        }
        return result;
    }
            ...
            throws IOException
    {
        ...
        if (pdfObject.getObject() == null)
        {
            ...
            {
                addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_MISSING_OFFSET,
            ...
            {
                addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_INVALID_OFFSET, "Object {" + objKey.getNumber()
            ...
            else if (offsetOrObjstmObNr > 0)
            {
                ...
                else
                {
                    ...
                    addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_OBJ_DELIMITER, "Single space expected [offset="+offset+"; key="+offsetOrObjstmObNr.toString()+"; line="+line+"; object="+pdfObject.toString()+"]"));
                    ...
                    for (char c : OBJ_MARKER)
                    {
                        if (source.read() != c)
                        {
                            addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_OBJ_DELIMITER, "Expected pattern '"
        ...
        if (offset > 0 && Arrays.equals(pattern, EOF_MARKER))
        {
            ...
            if (tmpOffset != buf.length)
            {
                        ...
                        || (buf.length - tmpOffset == 1 && (buf[tmpOffset] != 13 && buf[tmpOffset] != 10)))
                {
                    ...
                    addValidationError(new ValidationError(PreflightConstants.ERROR_SYNTAX_TRAILER_EOF,
