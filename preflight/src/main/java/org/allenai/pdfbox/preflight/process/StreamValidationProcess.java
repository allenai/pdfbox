...
public class StreamValidationProcess extends AbstractProcess
{
    ...
    protected void checkFilters(COSStream stream, PreflightContext context)
    {
        ...
        if (bFilter != null)
        {
            ...
            else
            {
                ...
                addValidationError(context, new ValidationResult.ValidationError(PreflightConstants.ERROR_SYNTAX_STREAM_INVALID_FILTER,
        ...
        try
        {
            ...
            if (offset != null)
            {
                while (skipped != offset)
                {
                    ...
                    if (curSkip < 0)
                    {
                        addValidationError(context, new ValidationResult.ValidationError(PreflightConstants.ERROR_SYNTAX_STREAM_DAMAGED, "Unable to skip bytes in the PDFFile to check stream length"));
                        return;
                    }
                    skipped += curSkip;
                }
    ...
    protected void checkDictionaryEntries(PreflightContext context, COSStream streamObj)
    {
        ...
        if (!len)
        {
            addValidationError(context, new ValidationResult.ValidationError(PreflightConstants.ERROR_SYNTAX_STREAM_LENGTH_MISSING,
                    "Stream length is missing"));
        }

        if (f || ffilter || fdecParams)
        {
            addValidationError(context, new ValidationResult.ValidationError(PreflightConstants.ERROR_SYNTAX_STREAM_FX_KEYS,
    ...
    {
        addValidationError(context, new ValidationResult.ValidationError(PreflightConstants.ERROR_SYNTAX_STREAM_LENGTH_INVALID,
