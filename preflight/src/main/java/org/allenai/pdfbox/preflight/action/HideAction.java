...
public class HideAction extends AbstractActionManager
{
    ...
    protected boolean innerValid()
    {
        ...
        if (!(COSUtils.isDictionary(t, cosDocument) || COSUtils.isArray(t, cosDocument) || COSUtils.isString(t,
                cosDocument)))
        {
            context.addValidationError(new ValidationResult.ValidationError(PreflightConstants.ERROR_ACTION_INVALID_TYPE, "T entry type is invalid"));
            return false;
        }
        ...
        if (h)
        {
            context.addValidationError(new ValidationResult.ValidationError(PreflightConstants.ERROR_ACTION_HIDE_H_INVALID, "H entry is \"true\""));
            return false;
        }
