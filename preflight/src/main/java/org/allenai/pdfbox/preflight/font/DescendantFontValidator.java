...
import org.allenai.pdfbox.pdmodel.font.PDCIDFont;
...
public abstract class DescendantFontValidator<T extends FontContainer> extends SimpleFontValidator<T>
{
    ...
    protected void checkCIDToGIDMap(COSBase ctog, boolean mandatory)
    {
        if (COSUtils.isString(ctog, cosDocument))
        {
            ...
            if (!PreflightConstants.FONT_DICTIONARY_VALUE_CMAP_IDENTITY.equals(ctogStr))
            {
                this.fontContainer.push(new ValidationResult.ValidationError(PreflightConstants.ERROR_FONTS_CIDKEYED_CIDTOGID,
