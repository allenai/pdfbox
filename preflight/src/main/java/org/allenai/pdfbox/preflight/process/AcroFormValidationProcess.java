...
import org.allenai.pdfbox.pdmodel.interactive.form.PDTerminalField;
...
import org.allenai.pdfbox.pdmodel.interactive.form.PDNonTerminalField;
import org.allenai.pdfbox.preflight.PreflightContext;


import static org.allenai.pdfbox.preflight.PreflightConfiguration.ANNOTATIONS_PROCESS;
...
public class AcroFormValidationProcess extends AbstractProcess
{
    ...
    public void validate(PreflightContext ctx) throws ValidationException
    {
        ...
        {
            ctx.addValidationError(new ValidationResult.ValidationError(PreflightConstants.ERROR_SYNTAX_NOCATALOG, "There is no Catalog entry in the Document"));
        }
    }
    ...
    {
        if (acroForm.getCOSObject().getBoolean(PreflightConstants.ACROFORM_DICTIONARY_KEY_NEED_APPEARANCES, false))
        {
            addValidationError(ctx, new ValidationResult.ValidationError(PreflightConstants.ERROR_SYNTAX_DICT_INVALID,
    ...
    protected boolean exploreFields(PreflightContext ctx, List<PDField> lFields) throws IOException
    {
        if (lFields != null)
        {
            ...
            for (Object obj : lFields)
            {
                ...
                {
                    addValidationError(ctx, new ValidationResult.ValidationError(PreflightConstants.ERROR_SYNTAX_BODY,
    ...
    protected boolean validateField(PreflightContext ctx, PDField field) throws IOException
    {
        ...
        if (aa != null)
        {
            addValidationError(ctx, new ValidationResult.ValidationError(PreflightConstants.ERROR_ACTION_FORBIDDEN_ADDITIONAL_ACTIONS_FIELD,
        ...
        if (field instanceof PDTerminalField)
        {
            ...
            if (res && widget != null)
            {
                ...
                if (act != null)
                {
                    addValidationError(ctx, new ValidationResult.ValidationError(PreflightConstants.ERROR_ACTION_FORBIDDEN_WIDGET_ACTION_FIELD,
