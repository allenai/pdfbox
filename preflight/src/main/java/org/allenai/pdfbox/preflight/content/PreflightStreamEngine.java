...
package org.allenai.pdfbox.preflight.content;
...
import org.allenai.pdfbox.pdmodel.graphics.color.PDCIEBasedColorSpace;
import org.allenai.pdfbox.pdmodel.graphics.color.PDICCBased;
import org.allenai.pdfbox.pdmodel.graphics.color.PDSeparation;
...
import org.allenai.pdfbox.preflight.graphic.ColorSpaces;
import org.allenai.pdfbox.preflight.graphic.ICCProfileWrapper;
import org.allenai.pdfbox.preflight.utils.FilterHelper;
import org.allenai.pdfbox.preflight.utils.RenderingIntents;
import org.allenai.pdfbox.contentstream.operator.DrawObject;
import org.allenai.pdfbox.contentstream.operator.Operator;
import org.allenai.pdfbox.contentstream.PDFStreamEngine;
...
public abstract class PreflightStreamEngine extends PDFStreamEngine
{
    ...
    protected void validateNumberOfGraphicStates(Operator operator) throws ContentStreamException
    {
        if ("q".equals(operator.getName()))
        {
            ...
            if (numberOfGraphicStates > PreflightConstants.MAX_GRAPHIC_STATES)
            {
    ...
    protected void validateInlineImageColorSpace(Operator operator) throws IOException
    {
        ...
        if (csInlinedBase != null)
        {
            if (COSUtils.isString(csInlinedBase, cosDocument))
            {
                ...
                if (cs == null)
                {
                    registerError("The ColorSpace " + colorSpace + " is unknown", PreflightConstants.ERROR_GRAPHIC_UNEXPECTED_VALUE_FOR_KEY);
                    return;
                }
            }
    ...
    private ColorSpaceHelper getColorSpaceHelper(PDColorSpace pdCS)
    {
        ...
        return csFact.getColorSpaceHelper(context, pdCS, ColorSpaceHelperFactory.ColorSpaceRestriction.ONLY_DEVICE);
    }
    ...
    protected void checkColorOperators(String operation) throws ContentStreamException
    {
                ...
                && !validColorSpace(cs, ColorSpaceType.RGB))
        {
            registerError("The operator \"" + operation + "\" can't be used with CMYK Profile",
                    PreflightConstants.ERROR_GRAPHIC_INVALID_COLOR_SPACE_RGB);
            return;
        }
                ...
                && !validColorSpace(cs, ColorSpaceType.CMYK))
        {
            registerError("The operator \"" + operation + "\" can't be used with RGB Profile",
                    PreflightConstants.ERROR_GRAPHIC_INVALID_COLOR_SPACE_CMYK);
            return;
        }
                ...
                && !validColorSpace(cs, ColorSpaceType.ALL))
        {
            registerError("The operator \"" + operation + "\" can't be used without Color Profile",
                    PreflightConstants.ERROR_GRAPHIC_INVALID_COLOR_SPACE_MISSING);
        }
    }
    ...
    protected void checkSetColorSpaceOperators(Operator operator, List<?> arguments) throws IOException
    {
        ...
        catch (IllegalArgumentException e)
        {
            ...
            if (pdCS != null)
            {
                ...
                PreflightConfiguration cfg = context.getConfig();
                ColorSpaceHelperFactory csFact = cfg.getColorSpaceHelperFact();
                csHelper = csFact.getColorSpaceHelper(context, pdCS, ColorSpaceHelperFactory.ColorSpaceRestriction.NO_RESTRICTION);
            }
        }

        if (cs == null)
        {
            registerError("The ColorSpace " + colorSpaceName + " is unknown", PreflightConstants.ERROR_GRAPHIC_UNEXPECTED_VALUE_FOR_KEY);
            return;
        }
